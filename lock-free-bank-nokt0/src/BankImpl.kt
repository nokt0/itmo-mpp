...
class BankImpl(override val numberOfAccounts: Int) : Bank {
    ...
    override fun withdraw(index: Int, amount: Long): Long {
        ...
        while (true) {
            val account = account(index)
            // Производим обновление только если нету других ожидающих операций
            if (account.invokeOperation())
                continue
            check(account.amount - amount >= 0) { "Underflow" }
            val updated = Account(account.amount - amount)
            // Обновление cas'ом, на случай если кто-то выполнил другую операцию параллельно
    ...
    private fun acquire(index: Int, op: Op): AcquiredAccount? {
        ...
        while (true) {
            ...
            // Нужно вернуть аккаунт если он уже ассоциирован с той же самой операцией
            if (account is AcquiredAccount && account.op == op) {
                return account
            }

            // Выполняем операцию для аккаунта если есть
            if (account.invokeOperation())
                continue
     ...
     */
    // Освобождает аккаунт от связи с операцией
     ...
     * @see acquire
     */
    // Аккаунт который связан с операцией, затрагивающей несколько аккаунтов
    ...
    private inner class TransferOp(val fromIndex: Int, val toIndex: Int, val amount: Long) : Op() {
        ...
        override fun invokeOperation() {
             ...
             * Basically, implementation of this method must perform the logic of the following code "atomically":
             */
            val minIndex = Integer.min(fromIndex, toIndex)
            val maxIndex = Integer.max(fromIndex, toIndex)

            // чтобы избежать цикла вызываем acquire всегда в порядке возрастания индекса
            acquire(minIndex, this)
            acquire(maxIndex, this)
